...
- Clone `git clone https://github.com/dangarfield/kujata-data.git` into a seperate directory and symlink into a previously created `ff7-fenrir` folder, ensuring that it is called `kujata-data`. Alternatively, edit `KUJATA_BASE` variable in `kernel-fetch-data.mj` to NOT point to localhost
