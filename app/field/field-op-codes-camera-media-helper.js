...
const getCurrentCameraPosition = () => {
  return {
    x: window.currentField.fieldCameraPosition.next.x,
    y: window.currentField.fieldCameraPosition.next.y
  }
}
...
const stopShake = async () => {
  ...
  const resetTween = new TWEEN.Tween(from, SHAKE_TWEEN_GROUP)
    .to({ x: 0, y: 0 }, time)
...
const shakeAxis = async (axis, amplitude, frames) => {
  ...
  for (let i = 0; i < steps.length; i++) {
    tweens.push(
      new TWEEN.Tween(from, SHAKE_TWEEN_GROUP)
        .to({ [axis]: amplitudeFactored * steps[i] }, time)
        .easing(TWEEN.Easing.Quadratic.InOut)
        .onUpdate(function () {
...
const scrollToEntity = async (entityId, tweenType, speed) => {
  const models = window.currentField.models.filter(
