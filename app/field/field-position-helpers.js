...
const updateArrowPositionHelpers = () => {
  ...
  if (
    ...
    for (
      let i = 0;
      i < window.currentField.positionHelpers.children.length;
      i++
    ) {
      if (
        window.currentField.positionHelpers.children[i].userData.type ===
        'animated'
      ) {
        animatedIds.push(i)
      } else if (
    ...
    if (animatedIds.length > 0) {
      const helper =
        window.currentField.positionHelpers.children[animatedIds[0]]
      if (helper.userData.frameCount >= 7) {
        helper.userData.frameCount = 0
        helper.userData.index =
          helper.userData.index >= 3 ? 0 : helper.userData.index + 1
      } else {
        helper.userData.frameCount++
      }
      for (const i in animatedIds) {
        window.currentField.positionHelpers.children[i].material.map =
          helper.userData.textures[helper.userData.index]
      }
    }
  ...
  if (
    ...
    for (
      ...
      if (
        ...
        const distance = window.currentField.fieldCamera.position.distanceTo(
          helperPosition
        )
        ...
        const helperOntopPoint = new THREE.Vector3().lerpVectors(
...
const drawArrowPositionHelpers = () => {
  ...
  for (
    let i = 0;
    i < window.currentField.data.triggers.gatewayArrows.length;
    i++
  ) {
    const arrowLocation = window.currentField.data.triggers.gatewayArrows[i]
    if (
      !(arrowLocation.x === 0 && arrowLocation.y === 0 && arrowLocation.z === 0)
    ) {
      // Not sure what shownArrows signifies yet, but its not always right
...
const drawArrowPositionHelper = (helperPosition, type) => {
  ...
  const finalDistance = window.currentField.fieldCamera.position.distanceTo(
    helperOntopPoint
  )
