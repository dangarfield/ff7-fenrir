...
const BTLMD = async op => {
  ...
  const options = []
  if (op.bits1 & 0b10000000) {
    options.push('DisableRewardScreens')
  }
  if (op.bits1 & 0b01000000) {
    options.push('ActivateArenaMode')
  }
  if (op.bits1 & 0b00100000) {
    options.push('DisableVictoryMusic')
  }
  if (op.bits1 & 0b00010000) {
    options.push('Unknown0b00010000')
  }
  if (op.bits1 & 0b00001000) {
    options.push('CanNotEscape')
  }
  if (op.bits1 & 0b00000100) {
    options.push('PreEmptiveAttack')
  }
  if (op.bits1 & 0b00000010) {
    options.push('TimedBattleWithoutRewardScreen')
  }
  if (op.bits1 & 0b00000001) {
    options.push('Unknown0b00000001')
  }
  if (op.bits2 & 0b11111110) {
    options.push('UnknownLSB')
  }
  if (op.bits2 & 0b00000001) {
    options.push('DisableGameOver')
  }
...
const BTMD2 = async op => {
  ...
  if (op.bits2 & 0b00000001) {
    options.push('NoCelebrations')
  }
  if (op.bits3 & 0b10000000) {
    options.push('DisableGameOver')
  }
  if (op.bits3 & 0b00000001) {
    options.push('DisableGameOver')
  }
...
const SPECIAL = async op => {
  ...
  switch (op.subOpName) {
    // Kernel based
    case 'ARROW':
      setFieldPointersEnabled(op.params[0] === 0)
      break
    case 'PNAME':
      window.alert(
        'opcode SPECIAL:PNAME used, there should be no instances of this'
      )
      break
    case 'GMSPD':
      window.alert(
        'opcode SPECIAL:GMSPD used, there should be no instances of this'
      )
      break
    case 'SMSPD':
      setConfigFieldMessageSpeed(op.params[1])
      break
    case 'BTLCK':
      setBattleLockEnabled(op.params[0] === 0)
      break
    case 'MVLCK':
      setMovieLockEnabled(op.params[0] === 0)
      break
    case 'SPCNM':
      setCharacterNameFromSpecialText(op.params[0], op.params[1])
      break
    case 'RSGLB':
      debugResetGame()
      break
