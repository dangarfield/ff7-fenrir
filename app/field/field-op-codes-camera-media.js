...
const SCRLA = async op => {
  // Scroll to entity // CHAR -> adds model.userData.entityId = entity script array i
...
const SCR2DC = async op => {
  // Scroll to position with smooth tween
...
const SCR2DL = async op => {
  // Scroll to position with linear tween
...
const SCRLP = async op => {
  ...
  const entities = window.currentField.models.filter(
    m => m.userData.characterName === memberName
  )
