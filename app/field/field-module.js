...
const setMenuEnabled = enabled => {
  window.currentField.menuEnabled = enabled
}
const isMenuEnabled = () => {
  return window.currentField.menuEnabled
}
...
const loadField = async (fieldName, playableCharacterInitData) => {
  ...
  const lastFieldName =
    window.currentField && window.currentField.name
  ...
  showLoadingScreen(
    playableCharacterInitData
      ? playableCharacterInitData.whiteTransition
      : false
  )
  resetTempBank()
  ...
  window.currentField.models = await loadModels(
    window.currentField.data.model.modelLoaders
  )
...
export { loadField, setMenuEnabled, isMenuEnabled }
