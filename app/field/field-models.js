...
const getModelByEntityId = entityId => {
  const modelsByEntityId = window.currentField.models.filter(
    m => m.userData.entityId === entityId
  )
  ...
  const modelsByEntityNameAndFieldModel = window.currentField.models.filter(
    m => m.userData.entityName === entityName
  )
  if (modelsByEntityNameAndFieldModel.length > 0) {
    console.log(
      'getModelByEntityId modelsByEntityNameAndFieldModel',
      modelsByEntityNameAndFieldModel
    )
...
const getModelByCharacterName = characterName => {
  return window.currentField.models.filter(
    m => m.userData.characterName === characterName
  )[0]
}
const getModelByCurrentPlayableCharacter = () => {
  return window.currentField.models.filter(
    m => m.userData.isPlayableCharacter === true
  )[0]
}
...
const setModelAsPlayableCharacter = (entityId, characterName) => {
  ...
  console.log(
    ...
    entityId,
    model,
    window.currentField.futureLeader
  )
  if (
    window.currentField.futureLeader !== undefined &&
    entityId === window.currentField.futureLeader
  ) {
...
const getFieldModelsForPlayableCharacter = characterId => {
  ...
  for (let i = 0; i < entities.length; i++) {
    ...
    if (entity.entityType !== 'Playable Character') {
      continue
    } // colne1 does not have Playable Character...
...
const positionPlayableCharacterFromTransition = async () => {
  ...
  if (window.currentField.playableCharacterInitData) {
    ...
    const modelDatas = getFieldModelsForPlayableCharacter(
      getPlayableCharacterId(window.data.savemap.party.members[0])
    )
    ...
    console.log(
      'positionPlayableCharacterFromTransition modelDatas',
      modelDatas
    )
    ...
    for (let i = 0; i < modelDatas.length; i++) {
      ...
      const { entityName, entityId, modelId } = modelData
      console.log(
        'positionPlayableCharacterFromTransition NO init data',
...
const placeModel = (entityId, x, y, z, triangleId) => {
  ...
  if (model === undefined) {
    ...
    model.scene.position.set(x / 4096, y / 4096, z / 4096)
    ...
    if (
      window.currentField.data.walkmeshSection.triangles[triangleId] ===
      undefined
    ) {
      console.log(
        'placeModel: x, y, triangleZ',
        entityId,
        'invalid triangle id',
        triangleId
      )
    ...
    model.scene.position.set(x / 4096, y / 4096, triangleZ / 4096)
    console.log(
      'placeModel: triangleId',
      entityId,
      triangleId,
      triangle,
      '->',
    ...
    if (
      ...
      console.log(
        'placeModel: triangleX, triangleY, triangleZ',
        entityId,
        'invalid triangle id',
        triangleId
      )
...
const placeModelsDebug = async () => {
  ...
  if (window.currentField.playableCharacterInitData) {
    ...
    let charFilter = window.currentField.models.filter(
      m => m.userData.characterName === 'Cloud'
    )
    if (charFilter.length === 0) {
      charFilter = window.currentField.models.filter(
        m => m.userData.isPlayableCharacter === true
      )
      if (charFilter.length === 0) {
    ...
    for (let i = 0; i < entities.length; i++) {
      ...
      if (entity.entityId === model.userData.entityId) {
        const triangleId = Math.round(
          window.currentField.data.walkmeshSection.triangles.length / 2
        )
        placeModel(
...
const setModelDirectionToFaceEntity = (entityId, targetEntityId) => {
  ...
  if (model === undefined || targetModel === undefined) {
    return
  }
  faceModelInstantly(model, targetModel)
}
const setModelDirectionToFaceCharacterOrPartyLeader = (
  entityId,
  characterId
) => {
  console.log(
  ...
  const characterNameFilter = window.currentField.models.filter(
    m => m.userData.characterName === characterName
  )
  let targetModel
...
const faceModelInstantly = (model, targetModel) => {
  ...
  const deg = getDegreesFromTwoPoints(
    model.scene.position,
...
const setModelAsLeader = entityId => {
  ...
  const model = getModelByEntityId(entityId)
  if (model !== undefined) {
    // Eg - CHAR codes have already set models to entities
    ...
    if (previousLeader && !entityHasPC) {
      console.log(
        'setModelAsLeader previous leader',
        previousLeader.userData,
        previousLeader.scene
      )
      ...
      model.scene.children[0].rotation.y =
        previousLeader.scene.children[0].rotation.y
      setModelVisibility(entityId, true)
    }
    ...
    // CHAR codes have not set the models to entities
...
const getDegreesFromTwoPoints = (point1, point2) => {
  return (
    180 +
    -1 *
      ((Math.atan2(point2.x - point1.x, point2.y - point1.y) * 180) / Math.PI)
  )
}
const turnModelToFaceEntity = async (
  entityId,
  entityIdToFace,
  whichWayId,
  steps
  ...
  const degrees = getDegreesFromTwoPoints(
    model.scene.position,
    modelToFace.scene.position
  )
...
const turnModelToFacePartyMember = async (
  entityId,
  partyMemberId,
  whichWayId,
  steps
...
const turnModelToFaceDirection = async (
  entityId,
  direction,
  whichWayId,
  steps,
...
const turnModel = async (entityId, degrees, whichWayId, steps, stepType) => {
  return new Promise(resolve => {
    ...
    const clockwiseDiff =
      desiredYDeg > currentYDeg
        ? 360 + currentYDeg - desiredYDeg
        : currentYDeg - desiredYDeg
    const antiClockwiseDiff =
      currentYDeg > desiredYDeg
        ? 360 + desiredYDeg - currentYDeg
        : desiredYDeg - currentYDeg
