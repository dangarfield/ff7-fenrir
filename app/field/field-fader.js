...
const setTransitionInProgress = progress => {
  transitionInProgress = progress
}

let fadeFader
let fadeInProgress = false
const isFadeInProgress = () => {
  return fadeInProgress
}
const setFadeInProgress = progress => {
...
const setTransitionFaderColor = newColor => {
  ...
  console.log(
    ...
    newColor,
    transitionFader.material.color
  )
}
    ...
    new TWEEN.Tween(from, FIELD_TWEEN_GROUP)
      ...
      .onUpdate(function () {
        ...
        if (from.hasOwnProperty('r')) {
          ...
          transitionFader.material.color = new THREE.Color(
            `rgb(${Math.floor(from.r)},${Math.floor(from.g)},${Math.floor(
              from.b
            )})`
          )
          console.log(
            'fadeTransition UPDATE: Color',
            window.currentField.fieldFader.material.color
          )
        }
        if (from.hasOwnProperty('o')) {
          transitionFader.material.opacity = from.o
          console.log(
            'fadeTransition UPDATE: Opacity',
            window.currentField.fieldFader.material.opacity
          )
        }
      })
      .onComplete(function () {
...
const fadeInstant = async (blendingType, colorType) => {
  ...
  fadeFader.material.color = new THREE.Color(
    `rgb(${Math.floor(colorType.r)},${Math.floor(colorType.g)},${Math.floor(
      colorType.b
    )})`
  )
...
const fade = (blendingType, from, to, end, timeMs) => {
  ...
  return new Promise(async resolve => {
    ...
    inProgressFade.to(to, timeMs).onUpdate(function () {
    ...
    inProgressFade.onComplete(function () {
      ...
      fadeFader.material.color = new THREE.Color(
        `rgb(${Math.floor(end.r)},${Math.floor(end.g)},${Math.floor(end.b)})`
      )
...
const removeInProgressFade = _id => {
  setFadeInProgress(false)
  inProgressFades = inProgressFades.filter(function (f) {
    return f._id !== _id
  })
}
