  ...
  for (let i = 0; i < window.currentField.data.script.entities.length; i++) {
    ...
    for (let j = 0; j < entity.scripts.length; j++) {
      ...
      for (let k = 0; k < script.ops.length; k++) {
        ...
        } else if (op.op === 'AKAO' || op.op === 'AKAO2') {
          // Just preload all param2-5 for simplicity
          if (op.p2 > 0 && op.p2 < 700) {
            loadSound(op.p2)
          }
          if (op.p3 > 0 && op.p3 < 700) {
            loadSound(op.p3)
          }
          if (op.p4 > 0 && op.p4 < 700) {
            loadSound(op.p4)
          }
          if (op.p5 > 0 && op.p5 < 700) {
            loadSound(op.p5)
          }
        } else if (op.op === 'PMVIE') {
          loadMovie(op.m)
...
const executeAkaoOperation = (akaoOp, p1, p2, p3, p4, p5) => {
  ...
  switch (akaoOp) {
    case 16:
    case 20: // 0x10 0x14 // Not used in fields
    ...
    case 21: // 0x15 // Not used in fields
      ...
      break
    case 24:
    case 25: // 0x18 0x19 // Not used in fields
    ...
    case 200: // 0xC8
      ...
      if (p1 < 128) {
        ...
        if (p1 < 30000) {
          // Works for md1stin, need to at others more
          setMusicVolume(0)
        } else {
          setMusicVolume(1)
        }
    ...
    case 201: // 0xC9
      ...
      if (p2 < 128) {
        ...
        console.log(
          `not sure what to do with this AKAO, should be musicPanTransition, but p2 is: ${p2}`
        )
      }
      break
    case 202: // 0xCA
      ...
      if (p2 < 128 && p3 < 128) {
        ...
        console.log(
          `not sure what to do with this AKAO, should be musicPanTransition, but p2 is: ${p2} - p3 is: ${p3}`
        )
      }
      break
