...
const isMoviePlaying = () => {
  return nextMovie.isPlaying
}
const isMovieLockEnabled = () => {
  return movieLockEnabled
}
...
const getMovieName = i => {
  ...
  console.log(
    ...
    i,
    movieMetadata,
    movieMetadata[`disc${disc}`],
    movieMetadata[`disc${disc}`][i]
  )
...
const setNextMovie = i => {
  ...
  if (potentialMoviecamList.length > 0) {
    ...
    nextMovie.doNotUseModels =
      nextMovie.cameraData.length ===
      nextMovie.cameraData.filter(f => f.xAxis.x === 0 && f.zoom === 0).length
  }
  nextMovie.frame = 0
  ...
  console.log(
    'setNextMovie moviecam',
    window.currentField.videoCamera,
