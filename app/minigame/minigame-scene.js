...
const renderLoop = () => {
    if (window.anim.activeScene !== 'minigame') {
        console.log('Stopping minigame renderLoop')
        return
    }
...
const startMiniGameRenderingLoop = () => {
    if (window.anim.activeScene !== 'minigame') {
        window.anim.activeScene = 'minigame'
        renderLoop()
    }
}
...
const setupScenes = () => {
    ...
    fixedCamera = new THREE.PerspectiveCamera(100, window.config.sizing.width / window.config.sizing.height, 0.001, 1000)
    fixedCamera.position.x = 5
    fixedCamera.position.y = 5
    fixedCamera.position.z = 5

    movingCamera = new THREE.PerspectiveCamera(100, window.config.sizing.width / window.config.sizing.height, 0.001, 1000)
    movingCamera.position.x = 10
    movingCamera.position.y = 20
    movingCamera.position.z = 30

    debugCamera = new THREE.PerspectiveCamera(100, window.config.sizing.width / window.config.sizing.height, 0.001, 1000)
    debugCamera.position.x = 10
    debugCamera.position.y = 20
    debugCamera.position.z = 30

    orthoCamera = new THREE.OrthographicCamera(0, window.config.sizing.width, window.config.sizing.height, 0, 0, 1001)
