...
const tempLoadDestinationSelector = async lastWMFieldReference => {
  ...
  for (let i = 0; i < metadata.length; i++) {
    ...
    const col = i - row * 3
    destination.scene = addDestinationText(
...
const navigateAndHighlight = change => {
  if (readyForNavigation) {
    ...
    console.log(
      'tempLoadDestinationSelector navigateAndHighlight',
      change,
      currentNavigation,
      metadata[currentNavigation]
    )
    highlightCurrentDestination()
  }
}
...
const navigateSelect = () => {
  if (readyForNavigation) {
    ...
    console.log(
      'tempLoadDestinationSelector navigateSelect',
      currentNavigation,
      destination
    )
    ...
    console.log(
      'tempLoadDestinationSelector loadField',
