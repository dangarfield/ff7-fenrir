...
const loadWorldMap = async lastWMFieldReference => {
  const transitionDataId = `${parseInt(lastWMFieldReference.replace('wm', '')) +
    1}`
  let transitionData = FIELD_TO_WORLD_DATA[transitionDataId]
  if (transitionData === undefined) {
    // TODO: some wmIds in field-id-to-world-map-coords.js do not have a destination
  ...
  // Temp
  await loadWorldMap2d(
    `${lastWMFieldReference} - meshX: ${transitionData.meshX}, meshY: ${transitionData.meshY} - SELECT DESTINATION`
  )
  // loadWorldMap3d()
  tempLoadDestinationSelector(lastWMFieldReference)
}

export { initWorldModule, loadWorldMap }
