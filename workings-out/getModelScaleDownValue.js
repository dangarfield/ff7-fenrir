...
const init = async () => {
  ...
  for (let i = 0; i < fields.length; i++) {
    ...
    if (
      fieldJson &&
      fieldJson.model &&
      fieldJson.model.header &&
      fieldJson.model.header.modelScale
    ) {
      console.log(
      ...
      scaleDatas.push({
        field: field,
        scale: fieldJson.model.header.modelScale
      })
    }
  }
  let grouped = _.chain(scaleDatas)
    .groupBy('scale')
    .map((k, v) => ({ scale: v, count: k.length, fields: k.map(n => n.field) }))
    .value()
  // console.log('grouped', grouped)
  // await fs.writeJson(OUT_FILE, grouped, { spaces: '\t'})
  let formattedOut = JSON.stringify(grouped)
    .replace(/\{/g, '\n\t{ ')
    .replace(/\,/g, ', ')
    .replace(/\:/g, ': ')
