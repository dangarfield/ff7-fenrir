        ...
        if (f && f.script && f.script.entities) {
            for (let j = 0; j < f.script.entities.length; j++) {
                ...
                for (let k = 0; k < entity.scripts.length; k++) {
                    ...
                    if (script.scriptType === '[OK]') { data.ok++; data.okOps = script.ops.length }
                    if (script.scriptType === 'Move' && script.index === 2) { data.move1++; data.move1Ops = script.ops.length }
                    if (script.scriptType === 'Move' && script.index === 3) { data.move2++; data.move2Ops = script.ops.length }
                    if (script.scriptType === 'Go') { data.go++; data.goOps = script.ops.length }
                    if (script.scriptType === 'Go 1x') { data.go1x++; data.go1xOps = script.ops.length }
                    if (script.scriptType === 'Go away') { data.goAway++; data.goAwayOps = script.ops.length }


                    for (let l = 0; l < script.ops.length; l++) {
                        const op = script.ops[l]
                        if (op.op === 'LINE') { data.line++ }
                    }
                }
                if (data.move1Ops > 0 && data.move2Ops > 0) { data.moveBothPresent = true }
                if (data.move1Ops > 1 && data.move2Ops > 1) { data.moveBothActive = true }

                if (data.goOps > 1 && data.go1xOps > 1) { data.goGo1xActive = true }
                if (data.goOps > 1 && data.goAwayOps > 1) { data.goGoAwayActive = true }
                if (data.go1xOps > 1 && data.goAwayOps > 1) { data.go1xGoAwayActive = true }
                if (data.goOps > 1 && data.go1xOps > 1 && data.goAwayOps > 1) { data.goAllActive = true }
